...
### 1. CSS Handles (Mandatory Styling)
To ensure merchants can customize your app via the Site Editor's CSS, you MUST use CSS handles for all visible elements.
```tsx
...
Never hardcode strings. Use `react-intl` and the `messages/` directory.
- **Local file**: `messages/en.json`, `messages/pt.json`
- **Usage**:
```tsx
import { FormattedMessage } from 'react-intl'
<FormattedMessage id="store/my-app.title" />
```

### 3. Admin Styleguide
If building an Admin page, you MUST use the `vtex.styleguide` library to maintain UI consistency.
```tsx
import { Layout, PageHeader, Button } from 'vtex.styleguide'
const AdminPage = () => (
  <Layout fullWidth pageHeader={<PageHeader title="App Config" />}>
